...
public class UpdateCamp extends Activity implements Cities {
	...
	public void toLogin() {
		Intent i = new Intent(UpdateCamp.this, Bntwrk.class);
		startActivity(i);
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		alertDialog  = new AlertDialog.Builder(UpdateCamp.this);
		alertDialog1 = new AlertDialog.Builder(UpdateCamp.this);
		alertDialog2  = new AlertDialog.Builder(UpdateCamp.this);
		...
		alertDialog.setMessage("Are you sure you want to update blood donation camp?");
				...
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						postParameters.add(new BasicNameValuePair("username",
								un.getText().toString()));
						postParameters.add(new BasicNameValuePair("campname",
								cn.getText().toString()));
						postParameters.add(new BasicNameValuePair("startday",
								sd.getText().toString()));
						postParameters.add(new BasicNameValuePair("starttime",
								st.getText().toString()));
						postParameters.add(new BasicNameValuePair("city", c));
						postParameters.add(new BasicNameValuePair("state",
								state));
						postParameters.add(new BasicNameValuePair("endday", ed
								.getText().toString()));
						postParameters.add(new BasicNameValuePair("etime", et
								.getText().toString()));
						String response = null;
						try {
							response = CustomHttpClient.executeHttpPost(
									"http://192.168.243.1:8084/Redx/udcamp.do",
									postParameters);
							String res = response.toString();
							res = res.replaceAll("\\s+", "");
							if (res.equals("0"))
								Toast.makeText(getApplicationContext(),
										"Blood Donation Camp does not exists..", Toast.LENGTH_SHORT).show();
							else if (res.equals("1"))
								Toast.makeText(getApplicationContext(),
										"Timing Error..", Toast.LENGTH_SHORT).show();
							else if (res.equals("2"))
								Toast.makeText(getApplicationContext(),
										"Blood Donation Camp Updated..", Toast.LENGTH_SHORT).show();
							else if (res.equals("4"))
								Toast.makeText(getApplicationContext(),
										"Invalid Entries..", Toast.LENGTH_SHORT).show();
							else
								Toast.makeText(getApplicationContext(),
		...
		alertDialog.setNegativeButton("NO",
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						Toast.makeText(getApplicationContext(),
								"Blood Donation Camp Updation Canceled", Toast.LENGTH_SHORT).show();
						dialog.cancel();
					}
				});
		...
		alertDialog2.setPositiveButton("YES",
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						Intent i = new Intent(UpdateCamp.this, Login.class);
						startActivity(i);
						finish();
					}
				});
